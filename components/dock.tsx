...
export default function Dock({ onNavigate, activeView }: DockProps) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>, id: string) => {
    ...
    // Refined bouncy animation
    const tl = gsap.timeline();
    tl.to(icon, { scale: 0.9, duration: 0.1, ease: "power2.inOut" }) // Quick press down
      .to(icon, { scale: 1.25, duration: 0.4, ease: "back.out(1.7)" }); // Pop up to the hover scale
  ...
  return (
    <footer ref={dockRef} className="fixed bottom-4 left-1/2 -translate-x-1/2">
